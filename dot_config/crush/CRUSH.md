...
- **NEVER use tools when giving information to users** - tool output is unreadable in Crush UI's small non-scrollable windows
- **ALL user dialogue must be normal text answers** after the thinking section closes
- **Thinking sections are for internal reasoning only** - no user-facing questions or content
- **Prefer conciseness** in normal answers, but longer scrollable responses are acceptable when needed
