;;; claude-code-ide-treesit-tools.el --- TreeSitter integration tools -*- lexical-binding: t; -*-
...
;; TreeSitter integration tools for Claude Code IDE.
...
(declare-function claude-code-with-temp-buffer "claude-code-ide-mcp-macros")
...
(define-mcp-tool treesit-analyze-structure (file-path)
    ...
    (progn
      (condition-case err
          (if (treesit-available-p)
              (claude-code-with-temp-buffer file-path
                (lambda ()
...
(define-mcp-tool treesit-get-node-at-point (file-path line column)
...
(define-mcp-tool treesit-find-parent-node (file-path line column node-type)
...
(define-mcp-tool treesit-get-children (file-path line column)
...
(provide 'claude-code-ide-treesit-tools)
;;; claude-code-ide-treesit-tools.el ends here