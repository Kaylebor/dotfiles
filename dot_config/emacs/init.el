...
;; project.el - Built-in project management (enhanced via ELPA)
(use-package project :ensure t
  :custom
  (project-vc-extra-root-markers '(".project" ".git" "Cargo.toml" "package.json" "mix.exs")))

;; Compatibility shims required by some completion packages (Vertico, etc.)
...
;; ANSI color auto-mode - Toggle ANSI color rendering automatically
(use-package ansi-color-auto-mode
  :ensure nil  ; Local package
  :load-path "~/.config/emacs/lisp/"
  ;; :custom
  ;; Optional: Set size limit for large log files (nil = no limit)
  ;; (ansi-color-auto-mode-size-limit "5M")
  :config
  ;; Enable globally for compilation, shell, eshell, and log files
  (global-ansi-color-auto-mode 1))

;; detached.el - Detached process management
;; Processes survive Emacs shutdown, unlimited scrollback, TRAMP support
;; Enables: compile, shell, eshell, vterm, dired, org, projectile integration
(use-package detached :ensure t
  :custom
  ;; ANSI color handling (detached.el manages this internally)
  (detached-filter-ansi-sequences t)
  ;; Show context when attaching to sessions
  (detached-show-session-context t)
  (detached-session-context-lines 20)
  ;; Terminal-data mode for full ANSI/color support (auto-detected per OS)
  (detached-terminal-data-command system-type)
  :config
  ;; Initialize all integrations (compile, shell, eshell, etc.)
  (detached-init)
  :bind
  ;; Remap compile globally - affects zig-compile, projectile-compile, etc.
  ([remap compile] . detached-compile)
  ([remap recompile] . detached-compile-recompile)
  ;; Session management - main benefit of detached.el
  ("C-c d o" . detached-open-session)
  ("C-c d d" . detached-detach-session))

;; icons - using nerd-icons for consistency
(use-package nerd-icons :ensure t
  :config
  (let ((bridge-file (expand-file-name "lsp-nerd-icons-bridge.el" user-emacs-directory)))
    (when (file-readable-p bridge-file)
      (load bridge-file nil 'nomessage))))
...
;; Pixel-perfect inertial scrolling with hardware deltas
...
(use-package treemacs-nerd-icons :ensure t
  ...
  (let ((bridge-file (expand-file-name "treemacs-nerd-icons-bridge.el" user-emacs-directory)))
    (when (file-readable-p bridge-file)
      (load bridge-file nil 'nomessage)))
...
(use-package consult :ensure t
  ...
  :config
  ;; Configure project buffer support
...
;; Custom VC-fd backend for fast file listing using fd
;; Wraps the built-in vc backend but uses 'fd' for file listing
(defun project-try-vc-fd (dir)
  "Try to detect a VC project for DIR, return vc-fd instance if fd is available."
  (when (executable-find "fd")
    (let ((vc-proj (project-try-vc dir)))
      (when vc-proj
        (list 'vc-fd (nth 1 vc-proj) (nth 2 vc-proj))))))

(cl-defmethod project-root ((project (head vc-fd)))
  "Return project root for vc-fd backend."
  (nth 2 project))

(cl-defmethod project-files ((project (head vc-fd)) &optional dirs)
  "Use fd for fast file listing in vc-fd projects."
  (mapcan
   (lambda (dir)
     (let ((default-directory (expand-file-name (file-name-as-directory dir))))
       (split-string (shell-command-to-string "fd . -0") "\0" t)))
   (or dirs (list (project-root project)))))

(cl-defmethod project-external-roots ((project (head vc-fd)))
  "Return external roots for vc-fd projects."
  nil)

;; Add with lower priority than project-try-vc (which is -100), so we wrap it
(add-hook 'project-find-functions #'project-try-vc-fd -90)
...
(use-package eat :ensure t
  ...
  (eat-mode . (lambda ()
                ...
                (setq-local eat-term-capabilities '(cursor unknown))
...
;; kkp helps with using terminal-based Emacs within Kitty
