...
**CRITICAL: Never call built-in tools within bash commands**: Built-in functions like `agent`, `fetch`, `glob`, `grep`, `edit`, `view`, etc. must be called directly as tools, not invoked inside bash shell strings. This is a common anti-pattern that will always fail.
